...
注意
- “Risk” はモデル内部の観測量（disempowerment potential proxy）
- “Satisfaction” は短期満足（thumbs-up相当の確率/スコア）
"""
...
def corr_safe(a: np.ndarray, b: np.ndarray) -> float:
    # Pearson correlation; return 0 if degenerate
    a = np.asarray(a)
    b = np.asarray(b)
    if a.size < 2:
        return 0.0
...
class Params:
    # Satisfaction model
    alpha_u: float = 2.0   # u contributes to satisfaction
    beta_r: float = 1.0    # reliance contributes to satisfaction
    gamma_d: float = 1.2   # difficulty reduces satisfaction
    delta_risk: float = 1.5

    # Reliance update
    k_r: float = 0.08      # reliance increase sensitivity
    k_d: float = 0.05      # reliance decrease via empowerment

    lam_a: float = 0.6     # (1-A) contributes to reliance increase

    # Autonomy update
    k_a: float = 0.02      # autonomy recovery rate via empowerment
    k_b: float = 0.06      # autonomy erosion rate via delegation

    # Risk proxy
    w_risk_A: float = 1.0
    w_risk_R: float = 1.0
    w_risk_D: float = 1.0
    risk_power: float = 1.0  # optional nonlinearity
    ...
    # 
    v_base: float = 0.10       # 平常時のvulnerability
    v_sigma: float = 0.05      # ゆらぎ
    v_spike_p: float = 0.01    # スパイク発生確率（ここで稀さを調整）
    v_spike_amp: float = 0.70  # スパイクの強さ
    th_V: float = 0.80         # severe判定用のvulnerability閾値



    # Success experience (proxy)
    eta0: float = 0.0        # baseline success logit
    etaA: float = 2.0        # higher autonomy -> more success
    etaD: float = 2.0        # higher difficulty -> less success
    etaU: float = 0.5        # more delegation -> slightly less learning-by-doing
@dataclass(frozen=True)
class Policy:
    name: str
    # For fixed policies
    ...
    def compute_u(self, A: float, R: float) -> float:
        ...
        # feedback: more help when A low, less help when R high
...
def simulate_trajectory(
    ...
    V = np.zeros(T, dtype=np.float32)          # ★FIX: ループ外で確保
    E = np.zeros(T, dtype=np.float32)          # success experience proxy (0..1)
    Severe = np.zeros(T, dtype=int)

    # 解析用：その時点の「Aが減り始める」R閾値（論文で使える）
    ...
    for t in range(T - 1):
        ...
        # Risk proxy: high when autonomy low, reliance high, difficulty high
        base_risk = (
        ...
        # ---- Update R (reliance)
        # A案 + (1-A): reliance grows with delegation (U) and low autonomy (1-A),
        # and is reduced when user does the work (1-U) via k_d.
        R[t + 1] = float(clip01(
            R[t]
            + params.k_r * U[t]
        ...
        # ---- Success experience (proxy)
        # 成功経験 = 「その時点でタスクを自力で解ける見込み」の近似（0..1）
        E_t = sigmoid_scalar(
            params.eta0
            + params.etaA * A[t]
            - params.etaD * D[t] 
        ...
        # ---- Update A (autonomy)
        # A(t+1) = A(t) - k_b * R(t) + k_a * SuccessExperience(t)
        erode = params.k_b * U[t] * R[t]   # ★侵食は「委任×依存」で起きる
        recover = params.k_a * E[t]        # ★成功経験で回復（まずはゲートなしでOK）
        A[t + 1] = float(clip01(A[t] + recover - erode))

        # ---- R_crit（ΔA=0境界）ログ：この値を超えると（他条件一定なら）Aは下がりやすい
        # ΔA=0 => R = (k_a * E_t) / k_b
        if params.k_b > 1e-12:
...
def _sigmoid(x: float) -> float:
    # overflowは通常ここでは起きにくいが、気になるなら分岐版にしてもOK
    return 1.0 / (1.0 + np.exp(-x))

@njit(cache=True, fastmath=True)
def simulate_population_delegating_jit(
    N: int,
    T: int,
    seed: int,
    ...
    for i in range(N):
        A = _clip01(A0_mean + A0_sd * rng.standard_normal())
        R = _clip01(R0_mean + R0_sd * rng.standard_normal())

        # 時系列は保持しない（統計のみ）
        for t in range(T):
            # difficulty
            D = _clip01(D_mu + D_sigma * rng.standard_normal())

            # vulnerability spike
            v = v_base + v_sigma * rng.standard_normal()
            if rng.random() < v_spike_p:
                v += v_spike_amp
            V = _clip01(v)
            ...
            # update A (あなたの最新版：recoverに(1-D))
            recover = k_a * (1.0 - U) * (1.0 - R) * (1.0 - D)
            erode   = k_b * U * R
    ...
    severe_rate = severe_cnt / max(n_tot, 1)
...
def simulate_population(
    ...
    # 返すなら最大 sample_cap だけ溜める（無限に溜めない）
    risk_buf = []
    sat_buf = []
...
def check_constraints(
    ...
    # C1: overall severe should be in target order; here we use high domain as conservative bound
    c1 = (target_low <= severe_rate_high <= target_high)
    # C2: high domain should have higher severe rate than low domain
    c2 = (severe_rate_high > severe_rate_low)
    # C3: risk and satisfaction positively correlated
...
def cmd_run(args: argparse.Namespace):
    # Domains: low vs high difficulty
    ...
    for pol in policies:
        ...
        pop_high = simulate_population(
            ...
            return_samples=need_samples,   # ★追加
            sample_cap=20000               # ★追加（任意）
        )
        ...
        # ★プロットする時だけ、risk_all/sat_all を参照する
        if need_samples:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # v_spike_p grid (logspaceを使うならstepsだけでOK)
